...
\begin{itemize}
\item Pour tout $v \in V$, $\left\lbrace \pi_K(k)v, k \in K \right\rbrace$ engendre un sous-espace de dimension fini,
\item Pour tout $v \in V$, l'action de $K$ sur $\left\lbrace \pi_K(k)v, k \in K \right\rbrace$ est continue,
...
\begin{definition}
Soient $f$ un coefficient $K$-fini à droite et à gauche de $\pi$ (obtenus à partir de vecteurs provenant du $(\mathfrak{g}, K)$-module sous-jacent) et $\phi : M_n(\mathbb{R}) \rightarrow \mathbb{C}$ une fonction de la forme
...
\label{thm_reel}
\begin{enumerate}
\item Il existe $s_0 \in \mathbb{R}$ tel que l'intégrale définissant la fonction zêta converge absolument pour tout $s \in \mathbb{C}$ vérifiant $Re(s) > s_0$.
\item Il existe $\nu_1, ..., \nu_n \in \mathbb{C}$ indépendants de $f$ et $\phi$ et des polynômes $\Xi(f,\phi,s)$ tels que
\begin{equation}
...
\item Il existe une fonction méromorphe $\gamma(s, \pi)$ indépendante de $f$ et $\phi$ telle que
\begin{equation}
...
\end{equation}
où $dx$ est une mesure de Haar sur $M_n(\mathbb{R})$ telle que $\hat{\hat{\phi}}(x) = \phi(-x)$.
\end{enumerate}
\end{theoreme}
...
\end{lemme}
La preuve est la même que dans le cas de $GL_1(\mathbb{Q}_p)$. Il nous suffit donc de faire le calcul explicite des membres de l'équation fonctionnelle pour des fonctions $\phi$ bien choisies.
...
\begin{align}
\gamma(s, |.|^t) = \frac{\pi^{-\frac{1-s-t}{2}}\Gamma(\frac{1-s-t}{2})}{\pi^{-\frac{s+t}{2}}\Gamma(\frac{s+t}{2})}, \\
\gamma(s, sgn(.)|.|^t) = i\frac{\pi^{-\frac{(1-s+t)+1}{2}}\Gamma(\frac{(1-s-t)+1}{2})}{\pi^{-\frac{s+t+1}{2}}\Gamma(\frac{s+t+1}{2})}.
\end{align}
...
\begin{theoreme}[de la sous-représentation \cite{wallach}]
